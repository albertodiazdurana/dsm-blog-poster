...
Some context on the pitch. This project generates what I call an Intrinsic Table
of Contents: a machine-readable map of a repository, its files, its sections, its
cross-references, and structural landmarks like hub files and orphaned documents.
Think of it as a README written for an LLM instead of a human, a briefing an agent
reads before it starts work so it knows where everything lives.

That briefing costs tokens. Every agent that reads it pays for it, on every task,
forever. And it was never meant to stay in one repository: the map was built here
first, on its way to every project in my Take AI Bite ecosystem, where the same
briefing would be read at the start of every session in every project. That
multiplication is what made this worth stopping for. A format decision affecting one
...
Here's the part I didn't examine. The test measured tokens. The whole decision was
framed around one number, context consumption, because tokens are easy to count and
easy to argue about. But cheaper is not the same as better. The table of contents
exists so an agent can navigate a repository, and I had chosen to judge formats by
their size, not by whether they actually helped an agent find anything. I'd picked
a proxy and mistaken it for the goal. The test was checking the wrong number.

## Scouting report: an adversarial fleet
...
to my own plan. The project keeps numbered experiment records the way it keeps decision
records, so this one has a name and a filed result: EXP-010.
...
## On the pitch: the A/B

The scouting report told me what to test. The next fleet, EXP-011, went and played the
match.
...
was glad to unmake it. The map goes on to the other projects it was always headed for,
in the format it started in, which is a duller outcome than the one I had planned and a
much better one to be sure of.
...
graph-retrieval world, leave the retrieval machinery behind. Community clustering, which
groups a repository's files by how densely they reference each other rather than by what
they say, is the structural half, and it is on the roadmap with no model-authored
summaries attached. The map gets richer; the agent stays the query engine.
