...
Enforcement is structural. A per-turn hook appends reasoning to a live transcript as the first tool call of every turn, so the record of thinking cannot be skipped by an eager agent. The checks guarding those appends recently gained the one they were missing: the timestamp an entry claims is now compared against the clock, so an entry logged hours off, or a retroactive note wearing the wrong time, is flagged rather than trusted. When a protocol violation is detected mid-output, the output itself is a stop condition: the agent halts, names the violation, and waits for confirmation before resuming. Before a Gate 2 approval, the agent is required to surface the strongest counter-evidence to its own recommendation, shifting the burden of critical evaluation from user vigilance to protocol.
...
Sessions are not "start working, stop working." Each session starts by checking inbox messages, validating project state, comparing versions, and loading context. Each session ends by saving memory, committing work, and creating continuity documents. If a session ends unexpectedly, the next session detects the gap and offers recovery. Continuity documents are now read before they are filed, not after: a handoff left for the next session is surfaced in the boot report alongside the checkpoint, and only then archived, so the sessions that most need the context stop losing it.
...
Destructive commands (force push, hard reset, recursive delete) require explicit human request. Sensitive files (.env, credentials) are blocked from commits. Feature branches must be tested before merging. The first write to any path outside the current repository in a session requires explicit user confirmation, so an agent working in a spoke project cannot quietly write into the Hub or any other project. That guard first covered only the file-writing tools; it now reaches shell commands too, and resolves the target the way the command itself would, so a redirect or a change of directory cannot slip a write past it.

The guardrails extend to publication. Merging a pull request against `main` is treated as equivalent in outcome to pushing to `main`, and both require specific confirmation. The same guard now refuses to open or merge a request while a commit sits unpushed, since that commit would be absent from the request, absent from the merge, and gone once the branch is deleted. At session start, the agent resolves the remote default branch and hard-halts if it differs from the local main line, so a stale session branch left as the repo default cannot quietly redirect a merge. Small checks, but the kind of thing that eats 45 minutes when it goes wrong.
...
The document suite kept filling gaps of its own. The planning templates gained a missing tier: a project that arrives already planned, a client build spec or a statement of work, now records the whole-project view in a template that points at the source and maps each milestone to real work, rather than restating it as a second plan that drifts. Guides earned an index and a way to gate themselves, opening by naming the conditions under which they apply and telling the reader to stop if those do not hold, so a lesson that is a landmine under one setup and noise elsewhere has a home.
...
Inheritance turned out leakier than it looked. Hook installation on a spoke had matched on the script name alone, discarding two of every three registrations as duplicates that never reached the project, and five slash commands were deploying to a folder only the hub could see, so every other project loaded a copy nothing had rewritten in months. Both had reported success throughout.
...
The record grew a distinction it had been missing: a log states what a pass actually found and is never rewritten, while an index asserts current status and is a defect when false, which is what finally answers whether a given file may be edited at all. A research file that claimed twenty-five sources confirmed over a pass that had checked thirteen is the overclaim it now catches. And a change to any document sweeps for every phrase it removed or renamed and reads each surviving mention, so a dangling reference or a stale self-count surfaces before the change ships rather than after.
...
One writing rule went the other way, from named to retired. A convention that turned a dash into a comma in anything a reader outside the project would see was first scoped to say which files it governed, then rescoped to follow the reader rather than the repository, and finally removed in favor of a skill that runs the same pass once against a finished document. A rule enforced on every write, by every author, forever is worth more run once on demand.
...
Spoke projects generate backlog proposals and methodology observations for the central repository. At session end, feedback is pushed to the central inbox. The inbox system routes notifications between projects with a processed/done lifecycle. The notification side of that loop gained a safety net: telling a downstream project about new features had been computed from a single session's changes, so anything a session failed to send stopped being a candidate the moment it ended, and session start now compares the full feature list against what the recipient actually received and reports the gap.
...
A large share of the newest features share one shape. They are not new powers but repairs to the methodology's own checks: a completeness check that could never report complete, a guard against colliding pull requests that had never once fired, a research run that reported success while returning an empty answer. Each had been passing while failing, because a wrong result looked exactly like a right one, and the fix each time was to make a check read the thing it actually guards. A methodology in active use catches its own silent failures, not only its users'.
