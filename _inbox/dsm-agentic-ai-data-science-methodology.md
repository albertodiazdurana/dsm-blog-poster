...
### [2026-09-04] FEATURES update from dsm-agentic-ai-data-science-methodology (S258, v1.26.0-v1.26.3)

**Type:** Feature Notification
**Priority:** Medium
**Source:** DSM Central

Five new features across four releases in one session. The through-line: a session-start
failure on the newest models was traced to a single phrase in an always-loaded instruction
block, the fix cascaded into a 43% reduction of that file, and a spoke then found that the
same fix had half-deleted an unrelated rule , which was retired rather than restored.
