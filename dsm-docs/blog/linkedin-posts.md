...
**URL:** https://www.linkedin.com/posts/albertodiazdurana_multiagent-humanaicollaboration-claudecode-share-7491033806167265280-IonJ/
**Status:** Published
**Blog post:** https://take-ai-bite.com/blog/2026-07-06-multi-agent-red-card/ (live; verified post-deploy with a cache-busted fetch, `age: 0`, seven distinct content properties confirmed including zero surviving "World Cup final")
...
**Slug observation (BL-023):** PREDICTION (recorded pre-publication): lead-3-hashtag-derived, `multiagent-humanaicollaboration-claudecode`. The opener ("One of my agents was asked how many orphaned documents a repository had") is not a clean keyword phrase, so the lead-hashtag branch was expected per the S26 refined hypothesis. Confidence deliberately low, as with Post 24: the S28 Post 15-vs-22 finding showed identical title and hashtag inputs producing divergent derivation, so this was a data point to collect rather than a forecast to rely on. **CONFIRMED:** actual slug `multiagent-humanaicollaboration-claudecode-share`, lead-3-hashtag-derived with "share" appended before activity id 7491033806167265280, the same shape as Posts 23 and 24. The stated basis held as well as the outcome: the opener was not a keyword phrase, which is the condition the hypothesis names, so this is a prediction on a declared basis rather than a lucky hit.

**Fourth consecutive clean prediction on the lead-hashtag branch (Posts 21, 23, 24, 25), all four with non-keyword-phrase openers. This strengthens the S26 refined hypothesis FOR THAT BRANCH ONLY and leaves the S28 non-determinism finding exactly where it was.** The distinction is the whole point of BL-023 and a streak is the moment it is most tempting to blur: all four predictions answer "which slug does the lead-hashtag branch produce, given the branch is taken". None of them tests "why is the branch taken at all". S28 remains the only case that bears on that question, because Posts 15 and 22 shared a title shape AND a hashtag set and still derived differently. A run of correct branch-conditional predictions can neither support nor refute a claim about determinism, so the open question is unchanged at four-for-four.
**Note:** 314 words as published. Leads with the 112-versus-15 orphan count, which the blog post buries in the middle of its A/B section, so the two pieces have different entry points and the cross-post does not read as a preview (S31 lead-asset rule). Register follows the owner's "anecdotal, not scientific" direction: one concrete number pair carries the story, and the measured percentages the post has available are deliberately left out. The blog post's closing line ("a decision instrument you cannot second-guess is not an instrument, it is a mirror") is NOT reused here, so it still lands fresh for a reader who follows the link. "an AI" singular, comma-not-em-dash.

**Author addition before posting, and a correction to the drafting rule as applied:** the final paragraph ("It was football season around the time I ran the experiment, so I had some fun framing it as a referee call: the agents red-carded my own decision") is the owner's, added on top of the drafted text. The draft deliberately carried no football reference at all, on the reasoning that the blog post opens on the World Cup metaphor and repeating it would make the cross-post a preview. That was an over-application: the S31 lead-asset rule protects the OPENING, and the draft extended it to the whole piece, which left a reader with no signal that the linked post is framed as a match report. Placing the metaphor in the closing position keeps the different entry point and restores the connective tissue to the blog post. Carry this distinction into future cross-posts, the rule is "do not share the lead", not "do not share the asset".
...
It was football season around the time I ran the experiment, so I had some fun framing it as a referee call: the agents red-carded my own decision.  
